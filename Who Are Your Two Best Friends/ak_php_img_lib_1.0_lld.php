...
//
function ak_img_watermark2($target, $wtrmrk_file,$wtrExt,$wtrmrk_file1,$wtrExt1,$wtrmrk_file2,$wtrExt2, $newcopy,$dp,$name,$name1,$name2,$dpExt,$userName) { 
     if ($wtrExt == "gif"){ 
    $watermark = imagecreatefromgif($wtrmrk_file);
    } else if($wtrExt =="png"){ 
    $watermark = imagecreatefrompng($wtrmrk_file);
	...
	if ($wtrExt2 == "gif"){ 
    $watermark2 = imagecreatefromgif($wtrmrk_file1);
    } else if($wtrExt =="png"){ 
    $watermark2 = imagecreatefrompng($wtrmrk_file1);
    } else { 
    $watermark2 = imagecreatefromjpeg($wtrmrk_file1);
    }	
	$dpExt = strtolower($dpExt);
    if ($dpExt == "gif"){ 
    $dpMark = imagecreatefromgif($dp);
   ...
   imagealphablending($watermark, false); 
    ...
    $wtrmrk_h = imagesy($watermark);
	 $wtrmrk_w1 = imagesx($watermark1); 
    $wtrmrk_h1 = imagesy($watermark1);
	$wtrmrk_w2 = imagesx($watermark2); 
    $wtrmrk_h2 = imagesy($watermark2);
	$dpmrk_w = imagesx($dpMark); 
    $dpmrk_h = imagesy($dpMark);
	$dst_x_anime = 1;
	$dst_y_anime = 40;
    ...
    imagecopy($img, $watermark, $dst_x_anime, $dst_y_anime, 0, 0, $wtrmrk_w, $wtrmrk_h); 
     /* $white = imagecolorallocate($img, 0, 0, 255);
	  ...
	  $dst_y_anime = 40;
	  imagecopy($img, $watermark1, $dst_x_anime, $dst_y_anime, 0, 0, $wtrmrk_w1, $wtrmrk_h1);
	  $dst_x_anime = $wtrmrk_w+4;
	   imagecopy($img, $watermark2, $dst_x_anime, $dst_y_anime, 0, 0, $wtrmrk_w2, $wtrmrk_h2);
		/*$text = "".$name1;
		$text_a = explode(' ', $text);
		$text = ''; //Empty String
		$width = $wtrmrk_w1;
